...
#SBATCH --array=0-99
#SBATCH --output=logs/active_%A_%a.out
#SBATCH --error=logs/active_%A_%a.err
...
echo "Running temporal VFE active episode ${SLURM_ARRAY_TASK_ID} on $(hostname)"
...
python scripts/minigrid/experiment.py \
    ...
    --record "$RECORD" \
    --output-dir data/minigrid/active \

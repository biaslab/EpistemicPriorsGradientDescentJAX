...
#SBATCH --array=0-99
#SBATCH --output=logs/marginal_%A_%a.out
#SBATCH --error=logs/marginal_%A_%a.err
...
echo "Running temporal VFE marginal episode ${SLURM_ARRAY_TASK_ID} on $(hostname)"
...
python scripts/minigrid/experiment.py \
    ...
    --record "$RECORD" \
    --output-dir data/minigrid/marginal \
